...
interface IExportToGithubEvent {
...
export const exportToGithub = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      const { projectId } = event.data.event.data as IExportToGithubEvent;
  ...
  async ({ event, step }) => {
    const { projectId, repoName, visibility, description, githubToken } =
      event.data as IExportToGithubEvent;
    ...
    const buildFilePaths = (files: TFileWithUrl[]) => {
      const fileMap = new Map<Id<"files">, TFileWithUrl>();

      files.forEach((f) => {
        fileMap.set(f._id, f);
      });

      const getFullPath = (file: TFileWithUrl): string => {
        if (!file.parentId) {
          return file.name;
        }
      ...
      const paths: Record<string, TFileWithUrl> = {};
      files.forEach((file) => {
